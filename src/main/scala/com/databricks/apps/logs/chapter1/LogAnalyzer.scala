...
import com.databricks.apps.logs.{SquidAccessLog, OrderingUtils}
import org.apache.spark.{SparkContext, SparkConf}
import org.apache.spark.SparkContext._

/**
 * The LogAnalyzer takes in an apache access log file and
 * The LogAnalyzer takes in an or some squid access log file and
...
//  val accessLogs = sc.textFile(logFile).map(ApacheAccessLog.parseLogLine).cache()
    val accessLogs = sc.textFile(logFile).map(SquidAccessLog.parseLogLine).cache()
